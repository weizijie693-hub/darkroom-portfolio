...
(function() {
    ...
    function renderGallery(photoList) {
        ...
        photoList.forEach((photo, index) => {
            ...
            if (photo.aspectRatio) {
                item.style.aspectRatio = photo.aspectRatio;
            }
    ...
    function showLightboxImage() {
        ...
        // Reset — remove all inline styles
        lightboxImg.removeAttribute('style');

        // Apply per-photo lightbox overrides
        if (photo.lightboxStyle) {
            const ls = photo.lightboxStyle;
            // Bypass parent constraints: position directly within the lightbox viewport
            lightboxImg.style.position = 'fixed';
            lightboxImg.style.left = '5vw';
            lightboxImg.style.top = '5vh';
            lightboxImg.style.width = '90vw';
            lightboxImg.style.height = '90vh';
            lightboxImg.style.objectFit = ls.objectFit || 'cover';
            lightboxImg.style.objectPosition = ls.objectPosition || 'right center';
            lightboxImg.style.zIndex = '10001';
        }
